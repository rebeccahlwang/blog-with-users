...
def register():
    form = RegisterForm()
    if form.validate_on_submit(): # request.method == "POST" does not work
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # X user = User.query.filter_by(email).first() -- causes an error message:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("post.html", post=requested_post, form=form, comment=comment, current_user=current_user)
